...
1 首先日志以行为单位读取和发送，每次发送的行数由服务端指定，不定长。</br> 2 第一此展开页面，先发送 startLine=0的请求，服务端如果没有获取到日志（startLine=0
lines=0），则间隔几秒继续刷日志startLine=0的请求</br>
如果客户端获取到日志（lines>0），进行步骤3 </br>
3 先把获取到的日志显示再浏览器上，间隔几秒可以继续发送请求 startLine=startLine+lines, </br>
...
6 在一次页面交互中，不保证每一次startLine=0获取到的初始日志是相同的，因为每次调用日志，日志文件都可能在滚动刷新，日志文件只记录第一次打开日志文件的快照，以及滚动刷新的部分。</br>
